...
class ShareLinkSerializer(serializers.ModelSerializer):
    """
    Serializer for creating and returning ShareLink objects.

    - `note` is read-only; it is assigned in the view based on the URL.
    - `token`, `access_count`, and `created_at` are read-only.
    - `expires_at` is optional and must be in the future if provided.
    """
    ...
    class Meta:
        model = ShareLink
        fields = ('id', 'note', 'token', 'expires_at', 'access_count', 'created_at')
        read_only_fields = ('id', 'note', 'token', 'access_count', 'created_at')
...
class PublicNoteSerializer(serializers.ModelSerializer):
    """
    Read-only serializer for public note access via share link.
    """
